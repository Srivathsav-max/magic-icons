...
const getOffsetIntersection = (
	...
	for (const o of [
		...
		const intersection = getIntersection(getOffsetLine(a1, a2, o[0]), getOffsetLine(b1, b2, o[1]));
...
const getClosestPointOnCircle = (circle: { x: number; y: number; r: number }, point: Point) => {
...
export default function arcify(svg: string) {
	...
	for (let idx = 0; idx < paths.length; idx++) {
		...
		const distance = Math.sqrt(
			(path.prev.x - prevPath.next.x) ** 2 + (path.prev.y - prevPath.next.y) ** 2,
		);
		...
		const closestPointOnLineA = getClosestPointOnLine(intersection, [prevPath.prev, path.prev]);
		const closestPointOnLineB = getClosestPointOnLine(intersection, [path.prev, path.next]);
		...
		for (let i = 0; i < paths.length; i++) {
			...
			if (isDistanceSmaller(paths[i].prev, prevCorner, 0.01)) {
				paths[i].prev = getClosestPointOnCircle({ ...intersection, r: radius }, paths[i].prev);
			}
			if (isDistanceSmaller(paths[i].next, prevCorner, 0.01)) {
				paths[i].next = getClosestPointOnCircle({ ...intersection, r: radius }, paths[i].next);
			}
		}
	}
