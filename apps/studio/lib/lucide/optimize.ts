...
import Commander, { type ASegment, type CSegment, type PathSegment } from "svg-path-commander";
...
const toPath = elementToPath as unknown as (element: unknown) => string;
	...
	for (let i = 0; i < children.length; i++) {
		...
		for (let i2 = 0; i2 < command.segments.length; i2++) {
			...
			if (segment[0] === "Z") {
				...
				nextPoint = { x: segment.at(-2), y: segment.at(-1) } as unknown as Point;
				lines.push({ prevPoint, nextPoint, id: i, idx: i2 });
			} else {
				nextPoint = { x: segment.at(-2), y: segment.at(-1) } as unknown as Point;
			}
	...
	for (let i = 0; i < data.children.length; i++) {
		if (data.children[i].name === "path") {
			...
			for (let j = 1; j < command.segments.length - 1; j++) {
				if (
					isDistanceSmaller(
						pathData.commands[j] as unknown as Point,
						pathData.commands[j - 1] as unknown as Point,
						0.05,
					)
				) {
...
const getArcFromCurve = (segment: CSegment, prevPoint: Point): ASegment | undefined => {
	...
	if (
		arcs.length === 2 &&
		(Math.abs(arcs[0].r - arcs[1].r) > 0.01 || isDistanceSmaller(arcs[0], arcs[1], arcs[0].r / 30))
	) {
		return undefined;
	}
		...
		if (c.name === "path") {
			...
			for (let i = 0; i < command.segments.length; i++) {
				...
				if (segment[0] === "Z") {
					...
					prevPoint = { x: segment.at(-2), y: segment.at(-1) } as unknown as Point;
				}
			}
		}
	});
		...
		if (data.children[i].name === "path") {
			...
			for (let i = 0; i < command.segments.length; i++) {
				...
				if (
					...
					prevPoint &&
					!isDistanceSmaller({ x: segment[6], y: segment[7] }, prevPoint, segment[1] * 2 - 0.001) &&
					segment[1]
				) {
			...
			for (let j = 0; j < segments.length; j++) {
				...
				if (
					...
					if (
						...
						const newCenter = isNewCenterPositive ? newCenterSweepPositive : newCenterSweepNegative;
...
const catchErrors = (fn: (svg: string) => string) => (svg: string) => {
	try {
		...
		stageWarningCount += 1;
		if (stageWarningCount <= MAX_STAGE_WARNINGS) {
			const message =
				(e as Error).message?.split("\n")[0]?.slice(0, 160) ?? "Unknown optimizer stage error";
			console.warn(
				`Lucide optimizer stage failed (${stageWarningCount}/${MAX_STAGE_WARNINGS}): ${message}`,
			);
			if (stageWarningCount === MAX_STAGE_WARNINGS) {
				console.warn("Further Lucide optimizer stage warnings will be suppressed.");
			}
		}
		return svg;
	}
					...
					for (let i4 = 0; i4 < lines.length; i4++) {
						if (lines[i4].id !== i || lines[i4].idx !== i2) {
							for (const offset of [0, 4, -4]) {
								const _offsetLine = getOffsetLine(lines[i4].prevPoint, lines[i4].nextPoint, offset);
								const offsetLine = {
								...
								if (intersection && isPointInLine(offsetLine, intersection)) {
									if (isDistanceSmaller(intersection, lines[i3].prevPoint, 1)) {
										...
										command.segments[i2 - 1][command.segments[i2 - 1].length - 2] = intersection.x;
										command.segments[i2 - 1][command.segments[i2 - 1].length - 1] = intersection.y;
										data.children[i].attributes.d = command.toString();
										break;
									}
...
export default memoize((value: string) => runOptimizations(runOptimizations(value)));
