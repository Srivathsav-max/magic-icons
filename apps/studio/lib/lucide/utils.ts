...
const toPathFn = toPath as unknown as (element: unknown) => string;
...
const convertToPathNode = (node: INode): { d: string; name: typeof node.name } => {
	...
	return { d: toPathFn(node).replace(/z$/i, ""), name: node.name };
};
	...
	for (let i = 0; i < commands.length; i++) {
		...
		switch (c.type) {
			case SVGPathData.MOVE_TO: {
				...
				start = { x: c.x ?? 0, y: c.y ?? 0 };
				break;
			}
			case SVGPathData.LINE_TO: {
				assert(prev);
				addPath(c, { x: c.x ?? 0, y: c.y ?? 0 });
				break;
			}
			case SVGPathData.HORIZ_LINE_TO: {
				assert(prev);
				addPath(c, { x: c.x ?? 0, y: prev.y });
				break;
			}
			case SVGPathData.VERT_LINE_TO: {
				assert(prev);
				addPath(c, { x: prev.x, y: c.y ?? 0 });
				break;
			}
			...
			case SVGPathData.CURVE_TO: {
				assert(prev);
				addPath(c, { x: c.x ?? 0, y: c.y ?? 0 }, `M ${prev.x} ${prev.y} ${encodeSVGPath(c)}`, {
					cp1: { x: c.x1 ?? 0, y: c.y1 ?? 0 },
					cp2: { x: c.x2 ?? 0, y: c.y2 ?? 0 },
				});
				break;
			}
					...
					x:
						...
						(previousCommand.type === SVGPathData.SMOOTH_CURVE_TO ||
							...
							? previousCommand.relative
								? (previousCommand.x2 ?? 0) - (previousCommand.x ?? 0)
								: (previousCommand.x2 ?? 0) - prev.x
							: 0,
					y:
						previousCommand &&
						(previousCommand.type === SVGPathData.SMOOTH_CURVE_TO ||
							previousCommand.type === SVGPathData.CURVE_TO)
							? previousCommand.relative
								? (previousCommand.y2 ?? 0) - (previousCommand.y ?? 0)
								: (previousCommand.y2 ?? 0) - prev.y
							: 0,
				};
				addPath(
					c,
					...
					if (previousCommand.type === SVGPathData.QUAD_TO) {
						return {
							x: previousCommand.relative
								? currentPoint.x - ((previousCommand.x1 ?? 0) - (previousCommand.x ?? 0))
								: currentPoint.x - ((previousCommand.x1 ?? 0) - currentPoint.x),
							y: previousCommand.relative
								? currentPoint.y - ((previousCommand.y1 ?? 0) - (previousCommand.y ?? 0))
								: currentPoint.y - ((previousCommand.y1 ?? 0) - currentPoint.y),
						};
					}
			...
			case SVGPathData.ARC: {
				...
				addPath(
					c,
					{ x: c.x ?? 0, y: c.y ?? 0 },
					`M ${prev.x} ${prev.y} A${c.rX ?? 0} ${c.rY ?? 0} ${c.xRot ?? 0} ${c.lArcFlag ?? 0} ${c.sweepFlag ?? 0} ${c.x ?? 0} ${c.y ?? 0}`,
					{ circle: (c.rX ?? 0) === (c.rY ?? 0) ? { ...center, r: c.rX ?? 0 } : undefined },
				);
				break;
			}
			default: {
...
function intersectTangents(start: Point, end: Point, center: Point): Point | undefined {
